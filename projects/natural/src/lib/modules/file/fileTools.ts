...
export function fixFileOrientationByMeta(file: File, result: orientationMeta): Promise<File> {
...
export function applyExifRotation(file: File): Promise<File> {
    if (file.type.indexOf('image/jpeg') !== 0) {
        return Promise.resolve(file);
    }

    return readOrientation(file).then((result: orientationMeta) => {
...
export function restoreExif(orig: any, resized: any) {
    ...
    ExifRestorer.slice2Segments = function (rawImageArray: number[]) {
        var head: number = 0,
            segments: number[][] = [];
    ...
    ExifRestorer.decode64 = function (input: any): number[] {
        var chr1,
            chr2,
            chr3: any = '',
            enc1,
